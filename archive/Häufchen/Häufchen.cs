...
class Häufchen
{
    private List<Karte> stapel = new List<Karte>();
    public List<Karte> Karten { get { return stapel; } set { stapel = value; } }
    ...
    public Aktion AddKarte(Karte karte)
    {
        ...
        else if ((GetRang(karte) == "König" && lenght == 12) || GetRang(karte).Contains("Joker") && stapel.Count == 12)
        {
            stapel.Add(karte);
            return Aktion.Voll;
        }
